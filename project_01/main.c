//  В начале программы нам предлагается выбрать режим работы программы:
...
//  3 4 + 2 - 3 ^ n
//  6 1 * 2 + n
//  (в конце каждой строки мы прописываем "n")
#include <stdio.h>
#include <stdlib.h>
#include "queue.h"
#include "stack.h"
...
//очередь
	case 'q':
		...
		scanf("%s", in);
		input = fopen(in, "r");
		queue_t queue = init_queue();                // Тут мы создаём пустую очередь.
		while (!feof(input))
			enqueue(&queue, read_string(input));     // Здесь мы добавляем в очередь строки.
		fclose(input);
